            ...
            <View style={{ flexDirection: 'row', marginVertical: 5 }}>
                ...
                <View>
                    <View style={styles.infoContainer}>
                        <TextView style={styles.status} text={isOpen ? 'Chưa xử lý' : 'Đã xử lý'} />
                        <Icon
                            color={isOpen ? 'green' : 'red'}
                            type="ionicon"
                            size={15}
                            name={isOpen ? "arrow-forward-circle-outline" : "close-circle-outline"}
                        />
                    </View>
                    ...
                    <View style={[styles.infoContainer]}>
                        <Icon
                            ...
                            style={{ marginRight: 4 }}
                        />
                        <TextView style={[styles.code, { marginVertical: 0 }]} text={user.displayName} />
                    </View>
                </View>
            </View>
            <View style={{ flexDirection: 'row' }}>
                <TextView style={styles.title} text={"Tên sự cố:"} />
                <TextView style={styles.normalText} numberOfLines={1} text={item.vidagis_incident_name} />
            </View>
            <View style={{ flexDirection: 'row' }}>
                <TextView style={styles.title} text={"Thời gian phát hiện:"} />
                <TextView style={styles.normalText} numberOfLines={1} text={moment(item.incurred_incident_str, "YYYY-MM-DDTHH:mm:ss").format("HH:mm DD/MM/YYYY")} />
            </View>
            <View style={{ flexDirection: 'row' }}>
                <TextView style={styles.title} text={"Thời gian khắc phục:"} />
                <TextView style={styles.normalText} numberOfLines={1} text={!isOpen ? moment(item.vidagis_handling_incident, "YYYY-MM-DDTHH:mm:ss").format("HH:mm DD/MM/YYYY") : "..."} />
            </View>
        </Pressable>
    );
};
